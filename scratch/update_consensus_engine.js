...
const targetPath = path.resolve('server/services/consensusEngine.js');
let fileContent = fs.readFileSync(targetPath, 'utf-8');

// Identify the start of generateSynthesis
const genIndex = fileContent.indexOf('export async function generateSynthesis(');
if (genIndex === -1) {
  console.error('generateSynthesis not found!');
  process.exit(1);
}

// Retain everything before generateSynthesis
const prefix = fileContent.slice(0, genIndex);

const newSynthesisCode = `export async function generateSynthesis(papers = [], query = '', consensus = null) {
...
const updatedContent = prefix + newSynthesisCode;
fs.writeFileSync(targetPath, updatedContent, 'utf-8');
console.log('Successfully updated consensusEngine.js with comprehensive 20-25 sentence synthesis engine!');
